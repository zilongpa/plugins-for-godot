...
@interface PHASEEngineWrapper () {
    ...
    std::atomic<int64_t> mResumeGeneration;
#endif //!TARGET_OS_OSX
}
...
#if !TARGET_OS_OSX
static const NSTimeInterval kAudioSessionActivationInitialRetryDelay = 0.05;
static const NSTimeInterval kAudioSessionActivationMaxRetryDelay = 1.0;
...
- (void)resumeRenderingAfter:(NSString*)reason delay:(NSTimeInterval)delay
{
    const int64_t generation = ++mResumeGeneration;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)),
                   mAudioSessionQueue,
                   ^{
        [self activateAudioSessionForReason:reason generation:generation attempt:1];
    });
}

- (void)activateAudioSessionForReason:(NSString*)reason generation:(int64_t)generation attempt:(NSUInteger)attempt
{
    if (mResumeGeneration.load() != generation)
    {
        NSLog(@"Phase Wrapper: Abandoning session activation for %@, superseded by a newer suspend/resume.", reason);
        return;
    }

    NSError *activateErr = nil;
    if ([[AVAudioSession sharedInstance] setActive:TRUE error:&activateErr])
    {
        NSLog(@"Phase Wrapper: Activated session on %@ after %lu attempt(s).", reason, (unsigned long)attempt);
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self->mResumeGeneration.load() == generation)
            {
                [self startRenderingAfter:reason];
            }
            else
            {
                NSLog(@"Phase Wrapper: Dropping rendering start for %@, superseded by a newer suspend/resume.", reason);
            }
        });
        return;
    }

    // Back off toward a heartbeat, since the app can become active long before the session is
    // reactivatable and there is no second notification to tell us when it is.
    const NSUInteger shift = MIN(attempt - 1, (NSUInteger)10);
    const NSTimeInterval delay = MIN(kAudioSessionActivationInitialRetryDelay * (double)(1u << shift), kAudioSessionActivationMaxRetryDelay);

    NSLog(@"Phase Wrapper: Failed to activate session on %@ (attempt %lu), retrying in %.0f ms: %@",
          reason, (unsigned long)attempt, delay * 1000.0, activateErr);

    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)),
                   mAudioSessionQueue,
                   ^{
        [self activateAudioSessionForReason:reason generation:generation attempt:attempt + 1];
    });
}

- (void)startRenderingAfter:(NSString*)reason
{
#if TARGET_OS_VISION
    if (sUseClientRenderingMode)
    {
    ...
    [self->mEngine startAndReturnError:&startErr];
    if (startErr) {
        NSLog(@"Phase Wrapper: engine restart on %@ failed: %@", reason, startErr);
    }
}
...
- (void)setupAudioSession
{
    ...
    mAudioSessionQueue = dispatch_queue_create("com.apple.phase.wrapper.audiosession", DISPATCH_QUEUE_SERIAL);
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidBecomeActiveNotification object:nil queue:nil usingBlock:^(NSNotification *notification)
     {
        [self resumeRenderingAfter:@"UIApplicationDidBecomeActiveNotification" delay:0.0];
    }];
    ...
    if (!sUseClientRenderingMode)
    {
        NSError *spatialErr = nil;
        if (![sessionInstance setIntendedSpatialExperience:AVAudioSessionSpatialExperienceBypassed options:nil error:&spatialErr])
        {
            NSLog(@"Phase Wrapper: Error setting intended spatial experience! %@", spatialErr);
        }
    }
#endif
...
- (void)handleAudioSessionInterruption:(NSNotification *)notification
{
    ...
    {
        AVAudioSessionInterruptionOptions options = (AVAudioSessionInterruptionOptions)[[notification.userInfo valueForKey:AVAudioSessionInterruptionOptionKey] unsignedIntegerValue];
        BOOL shouldResume = (options & AVAudioSessionInterruptionOptionShouldResume) != 0;

        [self resumeRenderingAfter:@"AVAudioSessionInterruptionTypeEnded" delay:shouldResume ? 0.0 : kAudioSessionActivationInitialRetryDelay];
    }
}
#endif // !TARGET_OS_OSX
